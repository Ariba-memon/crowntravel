      ...
      {blogPosts.map((item, index) => {
        return (
          ...
          >
            ...
            <p className="text-sm text-gray-700">{item.description.slice(0,100)}</p>
            <Link href={'/blogs/'+item.slug+"#blog"} className="text-stone-600 justify-end text-sm cursor-pointer absolute right-3 bottom-3">
              Read More
            </Link>
          </div>
        );
